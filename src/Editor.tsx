...
import { removeBlockFromTree, duplicateBlockInTree } from "./block-tree";
import {
  categoriesFor,
  categoryColor,
...
export default function Editor({
    ...
    const block = newBlock(locale, {
      ...
      title:
        kind === "group"
          ...
          : kind === "parallel"
            ? t("Travail en parallèle", "Parallel activities")
            : t("Note", "Note"),
      rooms:
        kind === "parallel"
          ? [
              {
                ...
                title: t("Salle 1", "Room 1"),
                blocks: [],
              },
              {
                id: crypto.randomUUID(),
                title: t("Salle 2", "Room 2"),
                blocks: [],
              },
            ]
          : undefined,
    });
    updateDay({ blocks: [...day.blocks, block] });
    showDetail(block.id);
  };
  ...
  const drop = (e: DragEvent, blockId: string) => {
    e.preventDefault();
    if (dragId.current) moveBlock(dragId.current, blockId);
    dragId.current = null;
  };
  return (
      ...
      <MentionProvider sessionId={session.id}>
        ...
        >
                ...
                <button
                  ...
                  onClick={() => {
                    const added = {
                      ...
                      date: new Date(
                        new Date(`${day.date}T12:00:00Z`).getTime() + 86400000,
                      )
                        .toISOString()
                        .slice(0, 10),
                      startTime: "09:00",
          ...
          <main className="editor-main">
            ...
            {status === "conflict" && (
              <div className="conflict-banner">
                ...
                <button className="button secondary small" onClick={data.load}>
            ...
            >
                ...
                <div className="day-settings">
                  ...
                  <label className="inline-field">
                    ...
                    <input
                      ...
                      readOnly={!editable}
                      onChange={(e) => {
                        if (e.target.value) updateDay({ date: e.target.value });
                      }}
                    />
                  </label>
                            ...
                            <>
                              ...
                              >
                                ...
                                {session.archived
                                  ...
                                  : t("Archiver la séance", "Archive session")}
                              </button>
                            </>
                          )}
                        </div>
              ...
              <div className="agenda-scroll" style={{ zoom: zoom / 100 }}>
                {editable && selectedBlocks.length > 0 && (
                  <div className="selection-toolbar">
                    <strong>
                      {selectedBlocks.length} {t("sélectionné(s)", "selected")}
                    </strong>
                    <select
                      aria-label={t(
                    ...
                    />
                    <button className="toolbar-button" onClick={copySelection}>
                      <Copy size={15} />
                    ...
                    {session.days.length > 1 && (
                      <>
                        <select
                          aria-label={t("Déplacer vers un jour", "Move to day")}
                          value={moveToDay}
                    ...
                    <button
                      className="icon-button"
                      aria-label={t("Annuler la sélection", "Clear selection")}
                ...
                >
                  ...
                  {scheduled.map(
                    (
                      ...
                      <div key={block.id} className="block-group">
                        ...
                        <div
                          className={`agenda-row category-${block.category} ${selection.has(block.id) ? "selected-row" : ""} ${containsActive(block) ? "current-block" : ""} ${conflict ? "schedule-conflict" : ""}`}
                          style={{
                          ...
                          onDrop={
                            editable ? (e) => drop(e, block.id) : undefined
                          }
                        >
                          ...
                          <div className="block-main">
                            <div className="block-title-row">
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  editBlock(block.id, { title: e.target.value })
                                }
                              />
                              <button
                  ...
                  {!day.blocks.length && (
                    <div className="agenda-empty">
                      ...
                      {editable && (
                        ...
                        >
                          <Plus size={17} />
                          {t("Ajouter le premier bloc", "Add the first block")}
                        </button>
                      )}
                    </div>
                  )}
                  ...
                  {day.blocks.length > 0 && (
                    <div className="agenda-end">
                      ...
                      <strong>{formatTime(scheduled.at(-1)!.endMinute)}</strong>
              ...
              <div className="editor-footer">
                ...
                <button onClick={() => setPanel("settings")}>
                  <Settings2 size={14} />
                  {t("Alertes sonores & paramètres", "Sound alerts & settings")}
                </button>
              </div>
            </div>
            {panel === "overview" && (
            ...
            {selectedPage && (
              <>
                <div className="page-alongside-toolbar">
                  ...
                  >
                    <Columns3 size={16} />
                    {t("Afficher à côté de l’agenda", "Show beside the agenda")}
                  </button>
                </div>
                <PageEditor
                  page={selectedPage}
          ...
          {currentBlock && (
            ...
            >
              <div className="block-detail-form">
                {editable && (
                  <button
                    className="toolbar-button"
                    onClick={() => {
                ...
                <div className="form-grid">
                  ...
                  <label>
                    ...
                    <input
                      ...
                      readOnly={!editable}
                      onChange={(e) =>
                        editBlock(currentBlock.id, { section: e.target.value })
                      }
                    />
                  </label>
                </div>
                ...
                {session.columns
                  .filter((c) => !["description", "facilitator"].includes(c.id))
                  .map((c) => (
                    <label key={c.id}>
                      <span>
                  ...
                  <div className="button-row">
                    ...
                    {currentBlock.lockedStart && editable && (
                      <button
                        className="button secondary small"
                        onClick={() =>
                          editBlock(currentBlock.id, { lockedStart: undefined })
                        }
                      >
